...
const BASE_URL = 'https://immigration-anywhere-be.onrender.com';
...
export const backend = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getGeolocation: builder.query<GeolocationResponse, string>({
      query: (payload) => ({
        url: `https://immigration-anywhere-be.onrender.com/google-maps-api`, //TODO https requests
        method: 'GET',
      ...
      async queryFn(arg, api, extraOptions, baseQuery) {
        ...
        if (createLocationResponse.error) {
          const getGeolocationIdResponse = await baseQuery({
            url: `https://immigration-anywhere-be.onrender.com/locations/location`, //TODO https requests
            method: 'GET',
            params: {
              locationName: arg.locationName,
              countryId: arg.countryId,
