...
const BASE_URL = 'https://immigration-anywhere-production.up.railway.app/';
...
export const backend = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getGeolocation: builder.query<GeolocationResponse, string>({
      query: (payload) => ({
        url: `https://immigration-anywhere-production.up.railway.app/google-maps-api`, //TODO https requests
        method: 'GET',
      ...
      async queryFn(arg, api, extraOptions, baseQuery) {
        ...
        if (createLocationResponse.error) {
          const getGeolocationIdResponse = await baseQuery({
            url: `https://immigration-anywhere-production.up.railway.app/locations/location`, //TODO https requests
            method: 'GET',
            params: {
              locationName: arg.locationName,
              countryId: arg.countryId,
